...
$params = array('q'          => array(XHProfLib::XHPROF_STRING_PARAM, ''),
...
if (!empty($run)) {
  ...
  $functions = XHProfLib::xhprof_get_matching_functions($q, $raw_data);

} else if (!empty($run1) && !empty($run2)) {

  // diff mode
  $raw_data = $xhprof_runs_impl->get_run($run1, $source, $desc_unused);
  $functions1 = XHProfLib::xhprof_get_matching_functions($q, $raw_data);

  $raw_data = $xhprof_runs_impl->get_run($run2, $source, $desc_unused);
  $functions2 = XHProfLib::xhprof_get_matching_functions($q, $raw_data);


  $functions = array_unique(array_merge($functions1, $functions2));
  asort($functions);
} else {
  XHProfLib::xhprof_error("no valid runs specified to typeahead endpoint");
  $functions = array();
}
