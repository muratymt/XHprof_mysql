...
namespace xhprof\lib\display;

use xhprof\lib\utils\iXHProfRuns;
...
class XHProf
{
    ...
    private static $diff_descriptions = [
        "fn" => "Function Name",
        "ct" => "Calls Diff",
        "Calls%" => "Calls<br>Diff%",
    ...
    public static function profiler_report(
        ...
        if (!empty($rep_symbol)) {
            $run1_data = XHProfLib::xhprof_trim_run($run1_data, [$rep_symbol]);
            if (self::$diff_mode) {
                $run2_data = XHProfLib::xhprof_trim_run($run2_data, [$rep_symbol]);
            }
        }
        ...
        $top_link_query_string = self::getBasePath().'/?'.http_build_query($base_url_params);
    ...
    public static function full_report($url_params, $symbol_tab, $sort, $run1, $run2)
    {
        ...
        if (self::$diff_mode) {
            ...
            $base_url_params = XHProfLib::xhprof_array_unset(XHProfLib::xhprof_array_unset($url_params,
                'run1'),
                'run2');
            $href1 = self::getBasePath().'/?'.
                http_build_query(XHProfLib::xhprof_array_set($base_url_params,
                    'run', $run1));
            $href2 = self::getBasePath().'/?'.
                http_build_query(XHProfLib::xhprof_array_set($base_url_params,
                    'run', $run2));
            ...
            print('<tr bgcolor="#bdc7d8" align=right>');
            print("<th></th>");
            print("<th $vwbar>".self::xhprof_render_link("Run #$run1", $href1)."</th>");
            print("<th $vwbar>".self::xhprof_render_link("Run #$run2", $href2)."</th>");
            ...
            if (self::$display_calls) {
                ...
                self::print_td_pct(self::$totals_2["ct"] - self::$totals_1["ct"], $totals_1["ct"], true);
                print('</tr>');
            }
            ...
            foreach (self::$metrics as $metric) {
                ...
                self::print_td_pct(self::$totals_2[$m] - self::$totals_1[$m], $totals_1[$m], true);
                print('<tr>');
            }
            print('</table>');
        ...
        print("<center><br><h3>".
            self::xhprof_render_link($callgraph_report_title,
                self::getBasePath()."/callgraph.php"."?".http_build_query($url_params))
            ."</h3></center>");
    ...
    public static function symbol_report(
        ...
        $callgraph_href = self::getBasePath()."/callgraph.php?"
        ...
        self::print_source_link(['fn' => $rep_symbol]);
        print("</td>");
