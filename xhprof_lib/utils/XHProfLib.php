...
class XHProfLib
{
     ...
     */
    public const XHPROF_STRING_PARAM = 1;
    public const XHPROF_UINT_PARAM = 2;
    public const XHPROF_FLOAT_PARAM = 3;
    public const XHPROF_BOOL_PARAM = 4;
    ...
    {
        XHProf::$diff_mode = $diff_report;

        if (!empty($sort)) {
            if (array_key_exists($sort, XHProf::$sortable_columns)) {
                XHProf::$sort_col = $sort;
            } else {
        ...
        if (!isset($xhprof_data["main()"]["wt"])) {
            ...
            if (XHProf::$sort_col == "wt") {
                XHProf::$sort_col = "samples";
            }
            ...
            XHProf::$display_calls = false;
        } else {
            XHProf::$display_calls = true;
        }

        // parent/child report doesn't support exclusive times yet.
        // So, change sort hyperlinks to closest fit.
        if (!empty($rep_symbol)) {
            XHProf::$sort_col = str_replace("excl_", "", XHProf::$sort_col);
        }

        if (XHProf::$display_calls) {
            XHProf::$stats = ["fn", "ct", "Calls%"];
        } else {
            XHProf::$stats = ["fn"];
        }

        XHProf::$pc_stats = XHProf::$stats;

        $possible_metrics = XHProfLib::xhprof_get_possible_metrics();
        foreach ($possible_metrics as $metric => $desc) {
            if (isset($xhprof_data["main()"][$metric])) {
                XHProf::$metrics[] = $metric;
                // flat (top-level reports): we can compute
                // exclusive metrics reports as well.
                XHProf::$stats[] = $metric;
                XHProf::$stats[] = "I".$desc[0]."%";
                XHProf::$stats[] = "excl_".$metric;
                XHProf::$stats[] = "E".$desc[0]."%";

                // parent/child report for a function: we can
                // only breakdown inclusive times correctly.
                XHProf::$pc_stats[] = $metric;
                XHProf::$pc_stats[] = "I".$desc[0]."%";
            }
        }
    }
     ...
     * in 2:4:1 ratio.
     *
     * @param iXHProfRuns $xhprof_runs_impl An object that implements
        ...
        foreach ($runs as $idx => $run_id) {
            ...
            if ($use_script_name) {
                ...
                if ($page) {
                    ...
                    $raw_data[XHProfLib::xhprof_build_parent_child_key("main()",
                        "__script::$page")]
                        = $fake_edge;
                } else {
                    $use_script_name = false;
                ...
                if ($use_script_name) {
                    ...
                    if (substr($parent_child, 0, 9) == "main()==>") {
                        ...
                        if (substr($child, 0, 10) != "__script::") {
                            $parent_child = XHProfLib::xhprof_build_parent_child_key("__script::$page",
                                $child);
                        }
                    }
                }
    ...
    public static function xhprof_normalize_metrics($raw_data, $num_runs)
    {
        ...
        if (isset($raw_data["==>main()"]) && isset($raw_data["main()"])) {
            XHProfLib::xhprof_error("XHProf Error: both ==>main() and main() set in raw data...");
        }
        ...
        foreach ($params as $k => $v) {
            switch ($v[0]) {
                case self::XHPROF_STRING_PARAM:
                    $p = XHProfLib::xhprof_get_string_param($k, $v[1]);
                    break;
                case self::XHPROF_UINT_PARAM:
                    $p = XHProfLib::xhprof_get_uint_param($k, $v[1]);
                    break;
                case self::XHPROF_FLOAT_PARAM:
                    $p = XHProfLib::xhprof_get_float_param($k, $v[1]);
                    break;
                case self::XHPROF_BOOL_PARAM:
                    $p = XHProfLib::xhprof_get_bool_param($k, $v[1]);
                    break;
                default:
                    XHProfLib::xhprof_error("Invalid param type passed to xhprof_param_init: "
                        .$v[0]);
                    exit();
            }
