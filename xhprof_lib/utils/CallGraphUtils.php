...
class CallGraphUtils
{
    ...
    public static function xhprof_generate_mime_header($type, $length)
    {
        ...
        if ($mime) {
            CallGraphUtils::xhprof_http_header('Content-type', $mime);
            CallGraphUtils::xhprof_http_header('Content-length', (string)$length);
        }
    }
    ...
    public static function xhprof_generate_dot_script(
        ...
        if ($critical_path) {
            $children_table = CallGraphUtils::xhprof_get_children_table($raw_data);
            $node = "main()";
            $path = [];
            $path_edges = [];
            $visited = [];
    ...
    public static function xhprof_get_content_by_run(
        ...
        $script = CallGraphUtils::xhprof_generate_dot_script($raw_data, $threshold, $source,
            $description, $func, $critical_path);

        $content = CallGraphUtils::xhprof_generate_image_by_dot($script, $type);
        return $content;
    }
    ...
    public static function xhprof_render_image(
        ...
        $content = CallGraphUtils::xhprof_get_content_by_run($xhprof_runs_impl, $run_id, $type,
            $threshold,
        ...
        CallGraphUtils::xhprof_generate_mime_header($type, strlen($content));
        echo $content;
    }
    ...
    public static function xhprof_render_diff_image(
        ...
        $children_table1 = CallGraphUtils::xhprof_get_children_table($raw_data1);
        $children_table2 = CallGraphUtils::xhprof_get_children_table($raw_data2);
        ...
        $script = CallGraphUtils::xhprof_generate_dot_script($run_delta, $threshold, $source,
            null, null, true,
