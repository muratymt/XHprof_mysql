...
$params = array(// run id param
                'run' => array(XHProfLib::XHPROF_STRING_PARAM, ''),

                // source/namespace/type of run
                'source' => array(XHProfLib::XHPROF_STRING_PARAM, 'xhprof'),

                // the focus function, if it is set, only directly
                // parents/children functions of it will be shown.
                'func' => array(XHProfLib::XHPROF_STRING_PARAM, ''),

                // image type, can be 'jpg', 'gif', 'ps', 'png'
                'type' => array(XHProfLib::XHPROF_STRING_PARAM, 'png'),

                // only functions whose exclusive time over the total time
                // is larger than this threshold will be shown.
                // default is 0.01.
                'threshold' => array(XHProfLib::XHPROF_FLOAT_PARAM, 0.01),

                // whether to show critical_path
                'critical' => array(XHProfLib::XHPROF_BOOL_PARAM, true),

                // first run in diff mode.
                'run1' => array(XHProfLib::XHPROF_STRING_PARAM, ''),

                // second run in diff mode.
                'run2' => array(XHProfLib::XHPROF_STRING_PARAM, '')
                );
...
if (!array_key_exists($type, CallGraphUtils::$xhprof_legal_image_types)) {
...
if (!empty($run)) {
  ...
  CallGraphUtils::xhprof_render_image($xhprof_runs_impl, $run, $type,
      $threshold, $func, $source, $critical);
} else {
  // diff report call graph image generation
  CallGraphUtils::xhprof_render_diff_image($xhprof_runs_impl, $run1, $run2,
